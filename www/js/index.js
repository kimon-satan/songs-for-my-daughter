...
`
  ...
  function deepChoose(arr) {
    return arr.splice(Math.floor(Math.random() * arr.length), 1);
  }
  function normalizeSum(inputArray) {
    const sum = inputArray.reduce((prev, curr) => prev + curr);
    return inputArray.map((n) => n / sum);
  }
  function randomArray(numItems, isNormalized) {
    const array = [];
  ...
  async function setup() {
    await start();
    Transport2.bpm = 100;
    masterChannel = new Channel({ channelCount: 2 });
    masterChannel.toDestination();
    setupMeter();
    new Loop(loop, "16n").start(0);
  }
  function setupMeter() {
    const meter = [new Meter(), new Meter()];
    ...
    const drawVolume = (dbs, x = 100) => {
      const amps = Math.pow(dbToGain(dbs), 0.5) * 200;
      context2.fillStyle = `rgb(255,0,0)`;
      context2.fillRect(x, canvas.height - amps, 100, amps);
      context2.fillStyle = `rgb(0,0,0)`;
      context2.fillText(amps, x + 50, 50);
    };
    const updateMeter = () => {
      const dbs = meter.map((m) => m.getValue());
      context2.clearRect(0, 0, canvas.width, canvas.height);
      context2.fillStyle = `rgb(0,0,255,0.25)`;
      context2.fillRect(0, 0, canvas.width, canvas.height);
      drawVolume(dbs[0], 75);
      drawVolume(dbs[1], 225);
      context2.fillStyle = `rgb(0,0,0)`;
      context2.fillText(`playbackBeat: ${playbackBeat}`, 0, 50);
      ...
      context2, window.requestAnimationFrame(updateMeter);
    };
    updateMeter();
  }
  function loop(time) {
  ...
  function playCurrentNote(note, time) {
    const panner = new Panner(note.pan).connect(masterChannel);
  ...
  function applyTransforms(_currentTransformState) {
    let _transformState = _currentTransformState ? { ..._currentTransformState } : initFillSequence();
  ...
  function getNextOctave(chroma, _currentTransformState) {
    ...
    if (isAscending) {
      if (Frequency(chroma + currOctave).toMidi() < Frequency(prevNote).toMidi()) {
        currOctave += 1;
      }
